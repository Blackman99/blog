...
export default {
  ...
  computed: {
    list() {
      const posts = this.$site.pages.filter(item => {
        if (item.pid !== 'post') {
          return false
        }
  ...
  methods: {
    isRelatedPost(item) {
      ...
      arrFn = Array.of(() => {
        if (typeof self.$frontmatter[keys[0]] === 'string' && typeof item.frontmatter[keys[0]] === 'string') { return self.$frontmatter[keys[0]] === item.frontmatter[keys[0]] }
      }, () => {
