<template>
  <div class="theme-main__inner archive">
    <ul class="archive__list">
      <li v-for="item of archiveList" :key="item.year" class="archive__item">
        <h2 class="archive__year">{{ item.year }}</h2>
        <div v-for="(subItem, key) of item.list" :key="key" class="archive__sub-item">
          <div class="archive__month-wrap">
            <span class="archive__month"> {{ key }} </span>
          </div>
          <div class="archive__leaf-list">
            <a v-for="leafItem in subItem" :key="leafItem.key" class="archive__leaf-item" :href="leafItem.path">
...
export default {
  ...
  computed: {
    archiveList() {
      ...
      list = list.sort((a, b) => {
        const time1 = new Date(a.frontmatter.date)
        const time2 = new Date(b.frontmatter.date)
        return time2 - time1
      })
      list.map(item => {
